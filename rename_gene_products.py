...
if args.database:
    ...
    print('Error: Please provide a name for name for your database')
...
MakeDB=[MAKEBLASTDB, ' -in ', args.database, ' -dbtype ', DB_TYPE, ' -parse_seqids ', '-out ', blast_dir,database_name]
...
with open(os.path.join(misc_dir, 'blast_out.txt'), 'r') as blast_out:
    for line in blast_out:
        ...
        if "|" in columns[1]:
            ...
            continue
        alignment_length = float((int(columns[3]) / int(columns[4])) * 100)
        if float(columns[2]) >= (args.identity * 100) and alignment_length >= args.align_length:
            filtered_blast[columns[0]] = columns[1]
...
with open(args.annotations, 'r') as anno:
    for line in anno:
        ...
        if columns[7] == 'hypothetical protein' or columns[7] == 'uncharacterized protein':
            hypo_genes.append(columns[0] + '-T1')
...
with open(args.database, 'r') as reference:
    for line in reference:
        if line.startswith('>'):
            ...
            if 'hypothetical protein' in ref_product or 'uncharacterized protein' in ref_product:
                continue
            else:
